    ...
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      {open && (
        <div style={{
          ...
          zIndex: 100, minWidth: 200, padding: '8px 0', overflow: 'hidden',
        }}>
          {children}
        </div>
      )}
...
export default function FilterBar({ onFilterChange, showDiscount = true, sizes = [] }: FilterBarProps) {
  ...
  function emit(patch: Partial<FilterState>) {
    onFilterChange({ discount, sort, priceMin, priceMax, size, ...patch })
  }

  function toggleDiscount() {
    const next = !discount; setDiscount(next); emit({ discount: next })
  }
  function setSort_(v: string) { setSort(v); emit({ sort: v }) }
  function setPrice(min: number | null, max: number | null) { setPriceMin(min); setPriceMax(max); emit({ priceMin: min, priceMax: max }) }
  function setSize_(s: string | null) { setSize(s); emit({ size: s }) }

  function reset() {
    setDiscount(false); setSort('popular'); setPriceMin(null); setPriceMax(null); setSize(null)
  ...
  const priceLabel = priceMin !== null || priceMax !== null
    ? priceMin === null ? `до ${priceMax?.toLocaleString()} ₽`
    : priceMax === null ? `от ${priceMin?.toLocaleString()} ₽`
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' as const, padding: '20px 0 28px' }}>
      {showDiscount && (
        <button onClick={toggleDiscount} style={{
          padding: '10px 18px', borderRadius: 999,
      ...
      <Dropdown label={sortLabel} active={sort !== 'popular'}>
        {SORT_OPTIONS.map(o => (
          <DropdownItem key={o.value} label={o.label} selected={sort === o.value} onClick={() => setSort_(o.value)} />
        ))}
      </Dropdown>

      <Dropdown label={priceLabel} active={priceMin !== null || priceMax !== null}>
        {PRICE_PRESETS.map(p => (
          <DropdownItem key={p.label} label={p.label}
            selected={priceMin === p.min && priceMax === p.max}
            onClick={() => setPrice(p.min, p.max)} />
        ))}
        {(priceMin !== null || priceMax !== null) && (
          <><div style={{ height: 1, background: 'var(--line)', margin: '6px 0' }} />
          <DropdownItem label="Сбросить цену" onClick={() => setPrice(null, null)} /></>
        )}
      </Dropdown>
      ...
      {sizes.length > 0 && (
        <Dropdown label={sizeLabel} active={size !== null}>
          {sizes.map(s => (
            <DropdownItem key={s} label={s} selected={size === s} onClick={() => setSize_(size === s ? null : s)} />
          ))}
          {size !== null && (
            <><div style={{ height: 1, background: 'var(--line)', margin: '6px 0' }} />
            <DropdownItem label="Сбросить размер" onClick={() => setSize_(null)} /></>
          )}
        </Dropdown>
      )}
