...
const PRODUCTS = [
  ...
  { id:3, name:'Купальник «Звезда»',     price:'от 9 800 ₽', emoji:'⭐', desc:'Тёмно-синий с золотыми кристаллами. Для торжественных выступлений.', cls:'3' },
  ...
  { id:6, name:'Купальник «Принцесса»',  price:'от 11 500 ₽', emoji:'🌸', desc:'Персиковый с розовыми стразами. Премиальная ткань, индивидуальный пошив.', cls:'6' },
  ...
  { id:8, name:'Купальник «Изумруд»',    price:'от 8 900 ₽', emoji:'💚', desc:'Насыщенный зелёный с золотой вышивкой. Благородный и элегантный.', cls:'8' },
  { id:9, name:'Купальник «Сапфир»',     price:'от 10 200 ₽', emoji:'💙', desc:'Глубокий синий с кристаллами и сеточными вставками. Современный крой.', cls:'9' },
]
...
export default function AtelierPage() {
  ...
  async function submitForm(e: React.FormEvent) {
    ...
    try {
      ...
      closeModal()
      setForm({ name:'', lastName:'', phone:'', email:'', size:'', date:'', comment:'' })
      setToast(true)
    ...
    <>
      ...
      <section className="page-hero compact">
        <div className="container">
          ...
          <div className="ph-eyebrow"><span className="dot"></span>54 ателье</div>
            ...
            {PRODUCTS.map(p => (
              <div className="prod-card" key={p.id}>
                <div className={`prod-card__img prod-card__img--${p.cls}`}>
                  {/* <img src={`/images/leotard-${p.id}.jpg`} alt={p.name} /> */}
                  {p.emoji}
                </div>
      ...
      {selected && (
        <div className="modal-overlay open" onClick={e => { if (e.target === e.currentTarget) closeModal() }}>
          <div className="modal">
            ...
            <div className="modal__title">Оставить заявку</div>
            <div className="modal__sub">Мы свяжемся с вами в течение 24 часов</div>
            <div className="modal__product">
              <div className="modal__product-emoji">{selected.emoji}</div>
              <div>
                <div className="modal__product-name">{selected.name}</div>
                <div className="modal__product-price">{selected.price}</div>
              </div>
            </div>
            ...
            <form onSubmit={submitForm}>
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input className="form-input" placeholder="Ваше имя" value={form.name} onChange={set('name')} />
                </div>
                <div className="form-group">
                  <label className="form-label">Фамилия</label>
                  <input className="form-input" placeholder="Фамилия" value={form.lastName} onChange={set('lastName')} />
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Телефон *</label>
                <input className="form-input" placeholder="+7 (___) ___-__-__" value={form.phone} onChange={set('phone')} />
              </div>
              <div className="form-group">
                <label className="form-label">Email</label>
                <input className="form-input" type="email" placeholder="example@mail.ru" value={form.email} onChange={set('email')} />
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Размер</label>
                  <input className="form-input" placeholder="30, 32, 34..." value={form.size} onChange={set('size')} />
                </div>
                <div className="form-group">
                  <label className="form-label">Дата соревнований</label>
                  <input className="form-input" type="date" value={form.date} onChange={set('date')} />
                </div>
              </div>
              <div className="form-group">
                <label className="form-label">Комментарий</label>
                <input className="form-input" placeholder="Пожелания, вопросы..." value={form.comment} onChange={set('comment')} />
              </div>
          ...
          <div className="modal">
            ...
            <form onSubmit={submitForm}>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <div className="field"><label>Имя *</label><input className="form-input" placeholder="Ваше имя" value={form.name} onChange={set('name')} readOnly style={{background:'#f5f5f5',cursor:'not-allowed'}} /></div>
                <div className="field"><label>Фамилия</label><input className="form-input" placeholder="Фамилия" value={form.lastName} onChange={set('lastName')} readOnly style={{background:'#f5f5f5',cursor:'not-allowed'}} /></div>
              </div>
              <div className="field"><label>Телефон *</label><input className="form-input" placeholder="+7 (___) ___-__-__" value={form.phone} readOnly style={{background:'#f5f5f5',cursor:'not-allowed'}} /></div>
              <div className="field"><label>Email</label><input className="form-input" type="email" placeholder="example@mail.ru" value={form.email} onChange={set('email')} readOnly style={{background:'#f5f5f5',cursor:'not-allowed'}} /></div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
